...
def write_debug_log(df, debug_file):
    ...
    for idx, row in df.iterrows():
        ...
        if last_end is not None:
            ...
            debug_file.write(f"Time diff: {(row['start_date'] - last_end).total_seconds() / 60:.2f} minutes\n")
...
def process_sleep_data(sleep_data, debug=False):
    ...
    sessions_to_exclude = df[df['sleep_type'].str.contains('Unspecified')]['session'].unique()
    ...
    for session in df['session'].unique():
        ...
        for _, row in session_df.iterrows():
            duration = (row['end_date'] - row['start_date']).total_seconds() / 3600  # in hours
...
def save_sleep_analysis(output_data, output_file='sleep_analysis.csv'):
    ...
    fieldnames = ['Sleep_Date', 'Total_Time_Asleep', 'Total_Deep_Sleep', 
                 'Total_Core_Sleep', 'Total_REM_Sleep', 'Total_Awake_Time',
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process Apple Health sleep data from XML export')
    parser.add_argument('--debug', action='store_true', help='Enable debug mode to generate detailed logging')
