...
outfile = f'../data/rolling_48hr_stress_{ts}.csv'
...
COPY (
  ...
  daily_stress AS (
    SELECT
      c.day,
      CAST(COALESCE(SUM(t.timetotalinhours * 60 * COALESCE(t.rpe, 5)), 0) AS INTEGER) as rolling_48h_stress
    FROM calendar c
    LEFT JOIN trainingpeaks_data t
      ON t.workoutday >= (c.day - INTERVAL '1 day')
