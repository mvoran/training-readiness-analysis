...
def check_upgradeable():
    ...
    stdout, stderr, code = run_command("pip install --upgrade --dry-run -e '.[dev]'", check=False)
                        ...
                        if line.startswith('Required-by:'):
                            ...
                            if required_by and required_by != 'None':
                                ...
                                for req_pkg in required_by.split(','):
                                    ...
                                    stdout2, stderr2, code2 = run_command(f"python -c \"import pkg_resources; reqs = pkg_resources.get_distribution('{req_pkg}').requires(); print('\\n'.join([str(r) for r in reqs if '{package}' in str(r)]))\"", check=False)
                                    if code2 == 0 and stdout2.strip():
                                        print(f"      {req_pkg} requires: {stdout2.strip()}")
                                    else:
                                        # Fallback to pip show
                                        stdout3, stderr3, code3 = run_command(f"pip show {req_pkg}", check=False)
                                        if code3 == 0:
                                            for line3 in stdout3.split('\n'):
                                                if line3.startswith('Requires:'):
                                                    requires = line3.split(':', 1)[1].strip()
                                                    if requires and requires != 'None' and package in requires:
                                                        print(f"      {req_pkg} requires: {requires}")
    else:
...
def update_dependencies(force=False):
    ...
    if code != 0:
        ...
        print("💡 Try running 'pip install --upgrade --force-reinstall -e .[dev]' to force resolution")
...
def full_check():
    ...
    actual_issues = sum([has_conflicts, has_sync_issues, has_health_issues, has_env_issues])
    if actual_issues > 0:
        ...
        if has_env_issues:
            print("💡 Run 'python scripts/manage_deps.py update' to fix environment issues")
    elif has_outdated:
        print("ℹ️  Some packages are outdated but constrained by dependencies (this is normal)")
...
def main():
    ...
    parser.add_argument("action", choices=["check", "update", "validate", "security", "sync", "constraints", "upgradeable", "full"],
                       help="Action to perform")
    parser.add_argument("--force", action="store_true", help="Force updates even if conflicts exist")
